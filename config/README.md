...
Dashboard 对 HR 只展示两个字段：

- 岗位名称
- 飞书岗位 ID

系统会根据当前岗位名称自动生成内部 `job_key`，并将 Boss 匹配名称固定为岗位名称本身，不再维护 Boss 岗位别名。

如需手工排查，也可以直接查看 `jobs.json`：
...
- `job_key`：项目内部标识，由系统根据岗位名称生成，不在 Dashboard 展示。
- `display_name`：Dashboard 展示名称，也是 Boss 页面精确匹配的岗位名称。
- `boss_job_names`：内部兼容字段，固定等于 `[display_name]`，不再由 HR 单独维护别名。
